...
async function rescueTokens({
  ...
  // Check bundler account balance
  let bundlerBalance = await targetClient.getBalance({
    address: bundlerAccount.address,
  });

  while (bundlerBalance < parseEther("0.001")) {
    await promptUser(
      `Fund bundler account (${bundlerAccount.address}) with native token on ${
        chain.name
      }.\nCurrent balance: ${formatEther(bundlerBalance)} ${
  ...
  console.log(
    `Bundler account funded (${bundlerAccount.address}). Proceeding...`
  );
  ...
  if (isNativeToken) {
    ...
    // Leave some for gas if transferring native token
    // P256/WebAuthn verification via FCL uses ~250k gas, plus execution overhead
    const gasBuffer = parseEther("0.5");
    if (balance > gasBuffer) {
      balance = balance - gasBuffer;
    }
  } else {
  ...
  console.log(
    `Transferring ${formatEther(balance)} ${tokenSymbol} to ${destination}`
  );
    ...
    : [
        {
          ...
          to: token!,
          args: [destination, balance] as const,
        },
      ];
...
`.trim();
  ...
  // P256/WebAuthn verification via FCL is expensive on Monad (~2M+ gas)
  // This is much higher than EVM simulation suggests
  const handleOpsGasLimit = BigInt(2_500_000);

  // Check bundler has enough balance for gas before submitting
  const gasPrice = await targetClient.getGasPrice();
  const requiredGasCost = handleOpsGasLimit * gasPrice;
